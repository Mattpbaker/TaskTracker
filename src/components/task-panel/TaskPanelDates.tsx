...
export default function TaskPanelDates({ startDate, dueDate, dueTime }: Props) {
  ...
  return (
    <div className="px-5 py-4 border-b border-border">
      <p className="text-[10px] uppercase tracking-widest text-muted font-semibold mb-3">Dates</p>
      <div className="flex gap-2.5">
        {startDate && (
          <div className="flex-1 bg-surface border border-border rounded-lg p-2.5">
            <p className="text-[9px] text-muted uppercase tracking-wide mb-1">Start</p>
            <p className="text-sm font-semibold text-primary">{formatDueDate(startDate)}</p>
          </div>
        )}
        <div className="flex-1 bg-surface border border-border rounded-lg p-2.5">
          <p className="text-[9px] text-muted uppercase tracking-wide mb-1">Due</p>
          <p className="text-sm font-semibold text-primary">{formatDueDate(dueDate, dueTime)}</p>
        </div>
